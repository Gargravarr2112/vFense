...
#################################### Get All Indexes ###################################################
	...
	downloaded_list = r.db(DB_NAME).table('downloaded_status').index_list().run(conn)
	custom_app_list = r.db(DB_NAME).table(AppCollections.CustomApps).index_list().run(conn)
	custom_app_per_agent_list = r.db(DB_NAME).table(AppCollections.CustomAppsPerAgent).index_list().run(conn)
	supported_app_list = r.db(DB_NAME).table(AppCollections.SupportedApps).index_list().run(conn)
	supported_app_per_agent_list = r.db(DB_NAME).table(AppCollections.SupportedAppsPerAgent).index_list().run(conn)
	vfense_app_list = r.db(DB_NAME).table(AppCollections.vFenseApps).index_list().run(conn)
	vfense_app_per_agent_list = r.db(DB_NAME).table(AppCollections.vFenseAppsPerAgent).index_list().run(conn)
	cve_list = r.db(DB_NAME).table(CVECollections.CVE).index_list().run(conn)
	windows_bulletin_list = r.db(DB_NAME).table(WindowsSecurityCollection.Bulletin).index_list().run(conn)
	ubuntu_bulletin_list = r.db(DB_NAME).table(UbuntuSecurityCollection.Bulletin).index_list().run(conn)
	files_list = r.db(DB_NAME).table(FileCollections.Files).index_list().run(conn)
	file_server_list = r.db(DB_NAME).table(FileCollections.FileServers).index_list().run(conn)
	tags_list = r.db(DB_NAME).table(TagsCollection).index_list().run(conn)
	agents_list = r.db(DB_NAME).table(AgentsCollection).index_list().run(conn)
	agent_operations_list = r.db(DB_NAME).table(OperationCollections.Agent).index_list().run(conn)
	admin_operations_list = r.db(DB_NAME).table(OperationCollections.Admin).index_list().run(conn)
	operations_per_agent_list = r.db(DB_NAME).table(OperationCollections.OperationPerAgent).index_list().run(conn)
	operations_per_app_list = r.db(DB_NAME).table(OperationCollections.OperationPerApp).index_list().run(conn)
	notif_list = r.db(DB_NAME).table(NotificationCollections.Notifications).index_list().run(conn)
	notif_history_list = r.db(DB_NAME).table(NotificationCollections.NotificationsHistory).index_list().run(conn)
	hw_per_agent_list = r.db(DB_NAME).table(HardwarePerAgentCollection).index_list().run(conn)
	tag_per_agent_list = r.db(DB_NAME).table(TagsPerAgentCollection).index_list().run(conn)
	notif_plugin_list = r.db(DB_NAME).table(NotificationCollections.NotificationPlugins,).index_list().run(conn)
	agent_queue_list = r.db(DB_NAME).table(QueueCollections.Agent).index_list().run(conn)
	groups_list = r.db(DB_NAME).table(GroupCollections.Groups).index_list().run(conn)
	groups_per_user_list = r.db(DB_NAME).table(GroupCollections.GroupsPerUser).index_list().run(conn)
	customer_per_user_list = r.db(DB_NAME).table(CustomerCollections.CustomersPerUser).index_list().run(conn)
...
#################################### DownloadStatusCollection Indexes ###################################################
	if not 'app_id' in downloaded_list:
		r.db(DB_NAME).table('downloaded_status').index_create('app_id').run(conn)

	if not 'by_filename_and_rvid' in downloaded_list:
		r.db(DB_NAME).table('downloaded_status').index_create(
