...
class vFenseLogger():
	...
	def _start_listener(self):
		try:
			print("Starting logging listener")
	...
	def _send_config(self, msg):
		...
		passed = True
		message = None
		print("Connecting to the listener at:")
		...
		try:
			...
			print(e, "Cannot connect to the listener, trying again")
			if self.count == 0:
				self.count = self.count + 1
				self._send_config(msg)
			passed = False
			message = e.message + ' ' + self.LISTENER_HOST + ' ' +  str(self.LISTENER_PORT)
		if passed:
			print("Sending the new config file")
			try:
				sender.send(bytes(struct.pack('>L', len(msg))))
				sender.send(msg)
